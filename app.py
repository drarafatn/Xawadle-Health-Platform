...
with st.sidebar:
    ...
    section = st.radio("View", ["Executive overview", "Service analytics", "Nutrition & EPI", "Biostatistics", "AI benchmark"])
    st.divider()
...
with st.expander("Data quality and provenance", expanded=not quality.passed):
    ...
    st.caption("Missing values are preserved as NA. The source prompt omitted numeric disease-by-gender counts and IPV/Penta totals.")
...
if section == "Executive overview":
    ...
    st.info("The Q3 picture is dominated by a high under-5 service load and a 25.0% malnutrition rate among screened children. The strongest immediate analytics priority is completing disease-level, gender-disaggregated registers so respiratory, diarrhoeal, and immunization hypotheses can be tested rather than assumed.")
    c1,c2=st.columns(2)
    with c1:
        st.markdown("#### Service volume")
        chart=services[services.total.notna() & services.indicator.isin(["OPD over 5 years","OPD under 5 years","ANC","PNC","Deliveries"])].copy()
        fig,ax=plt.subplots(figsize=(7,4)); sns.barplot(data=chart,y="indicator",x="total",palette="crest",ax=ax); ax.set(xlabel="Reported count",ylabel=""); st.pyplot(fig, clear_figure=True)
    with c2:
        st.markdown("#### Priority root-cause actions")
        st.dataframe(root_cause_actions()[["driver","indicator","action"]], use_container_width=True, hide_index=True)

elif section == "Service analytics":
    st.markdown("## Service analytics")
    chart=services[services.total.notna()].copy()
    fig,ax=plt.subplots(figsize=(10,5)); sns.barplot(data=chart,y="indicator",x="total",hue="domain",dodge=False,palette="viridis",ax=ax); ax.set(xlabel="Reported count",ylabel=""); st.pyplot(fig, clear_figure=True)
    st.dataframe(chart, use_container_width=True, hide_index=True)
    st.markdown("### Gender disaggregation")
    gender=services[services.male.notna() & services.female.notna()][["indicator","male","female"]].melt("indicator",var_name="sex",value_name="count")
    fig,ax=plt.subplots(figsize=(9,4)); sns.barplot(data=gender,x="indicator",y="count",hue="sex",palette={"male":"#2563eb","female":"#db2777"},ax=ax); ax.tick_params(axis="x",rotation=35); st.pyplot(fig, clear_figure=True)
    ...
    with left:
        ...
        fig,ax=plt.subplots(figsize=(7,5)); sns.barplot(data=nut,y="indicator",x="total",palette="mako",ax=ax); ax.set(xlabel="Count",ylabel=""); st.pyplot(fig, clear_figure=True)
        st.metric("Malnutrition among screened", pct(metrics["malnutrition_rate"]))
    with right:
        e=epi[epi.total.notna()].copy(); e["coverage"] = e.total/300
        fig,ax=plt.subplots(figsize=(7,5)); sns.barplot(data=e,y="antigen",x="coverage",palette="rocket",ax=ax); ax.set(xlabel="Coverage against stated denominator",ylabel=""); ax.set_xlim(0,1.05); st.pyplot(fig, clear_figure=True)
    ...
    st.warning("These are aggregate proxy comparisons, not causal or patient-level disease risk estimates. Linked numerator/denominator cohorts are required for valid clinical inference.")
